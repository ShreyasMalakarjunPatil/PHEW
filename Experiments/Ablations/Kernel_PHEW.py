...
from Prune import kernel_phew_utils
import copy
...
def run(args):
    ...
    prob, reverse_prob = kernel_phew_utils.generate_probability(model, verbose = True)

    net = copy.deepcopy(model)
    for p in net.parameters():
        p.data.fill_(0)
    weight_masks, bias_masks = kernel_phew_utils.generate_masks(net)
    ...
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = kernel_phew_utils.phew_masks(model, prune_perc, prob, reverse_prob, weight_masks,bias_masks,verbose=True,kernel_conserved=True)
