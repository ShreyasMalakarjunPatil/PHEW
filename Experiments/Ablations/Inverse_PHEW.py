...
def run(args):
    ...
    prob, reverse_prob, kernel_prob = biased_randwalk_utils.generate_probability(model, verbose = True)
    ...
    for p in net.parameters():
        ...
        if len(p.data.size()) != 1:
            num = num + 1
    weight_masks, bias_masks = biased_randwalk_utils.generate_masks(net)
    results = []
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = biased_randwalk_utils.iphew_masks(model, prune_perc, prob, reverse_prob,kernel_prob, weight_masks,bias_masks,verbose=True,kernel_conserved=False)
