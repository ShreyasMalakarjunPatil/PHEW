...
def run(args):
    ...
    prob, reverse_prob, kernel_prob = phew_utils.generate_probability(net, verbose=True)
    ...
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = phew_utils.phew_masks(model, prune_perc, prob, reverse_prob, kernel_prob,weight_masks,bias_masks,verbose=True)
        ...
        with open(args.ablation_experiment + str(args.prune_perc[len(args.prune_perc)-i-1]) + str(args.model) + str(args.dataset) +str(args.seed)+ '_Mask.pkl', "wb") as fout2:
