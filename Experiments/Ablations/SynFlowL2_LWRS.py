...
def run(args):
    ...
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = synflowl2_utils.synflowl2_prune_masks(sparse_model, prune_perc, prune_loader, args.synflow_iterations, dev)
        wm = copy.deepcopy(weight_masks)
        wm, bm = Utils.layerwise_randomshuffle(sparse_model, wm, 1.0, dev)
