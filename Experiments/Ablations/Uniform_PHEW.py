...
from Prune import uniform_randwalk_utils
import copy
...
def run(args):
    ...
    for p in net.parameters():
        p.data.fill_(0)
    weight_masks, bias_masks = uniform_randwalk_utils.generate_masks(net)
    results = []
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = uniform_randwalk_utils.uniform_masks(model, prune_perc, weight_masks, bias_masks,
                                                                        verbose=True, kernel_conserved=False)
        ...
        (trained_model, train_curve, test_loss, accuracy1, accuracy5) = train.train(sparse_model, loss, optimizer,
                                                                                    train_loader,
                                                                                    test_loader, dev, args.epochs,
                                                                                    scheduler)
        ...
        with open(args.ablation_experiment + str(args.prune_perc[len(args.prune_perc) - i - 1]) + str(args.model) + str(
                args.dataset) + str(args.seed) + '.pkl', "wb") as fout:
            pkl.dump(results, fout, protocol=pkl.HIGHEST_PROTOCOL)

        with open(args.ablation_experiment + str(args.prune_perc[len(args.prune_perc) - i - 1]) + str(args.model) + str(
                args.dataset) + str(args.seed) + '_Mask.pkl', "wb") as fout1:
