...
def run(args):
    ...
    for i in range(len(args.prune_perc)):
        ...
        weight_masks, bias_masks = phew_utils.phew_masks(model, prune_perc, prob, reverse_prob, kernel_prob,
                                                         weight_masks, bias_masks, verbose=True)

        sparse_model._initialize_weights_evci(weight_masks)
