...
def layerwise_randomshuffle(net,weight_masks,fraction,dev):
    ...
    for i in range(len(weight_masks)):
        ...
        ratio1 = 100.0 - 100.0*torch.sum(weight_masks[i])/torch.numel(weight_masks[i])
        width = 0
        ...
        for j in range(len(scores[i])):
            if j<width:
                scores[i][j] = 100.0*torch.abs(torch.randn(scores[i][j].size())) #+ 100.0 * torch.ones(scores[i][j].size())
                all_weights += list(scores[i][j].cpu().detach().data.abs().numpy().flatten())
            else:
                scores[i][j] = torch.zeros(scores[i][j].size())
                all_weights += list(scores[i][j].cpu().detach().data.abs().numpy().flatten())
        threshold = np.percentile(np.array(all_weights), ratio1.cpu().detach().numpy())
